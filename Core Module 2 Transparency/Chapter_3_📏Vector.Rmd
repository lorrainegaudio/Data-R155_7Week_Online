...
![R Notebook Code Chunk play button](play_button.png)
...
# Step 1: create the exponent values
exponents <- ___:___

# Step 2: raise the base to each exponent value
binary_powers <- ___ ^ exponents
```
...
🗣 Add Comments:  "# The output shows ___ elements (values) in the `binary_powers` vector."

🗣 How does R applies the exponentiation operator `^`? Why does the output producing __ elements (values) in the `binary_powers` vector? Does the vector start with 1 and grow by doubling? 
...
🎯 In one sentence, explain why numeric values may be coerced to character values if you mix them inside a single vector.

🗣 Your explanation: # Numeric values may be coerced to character values because __________.
