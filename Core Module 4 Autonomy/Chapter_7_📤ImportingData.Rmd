...
✅ In the console, look directly under the `getwd()` and `list.files()` chunk. Confirm that the folder path matches where you think the working directory is located.

🗣 When I run `getwd()`, I am checking where R is currently "looking." When I run `list.files()`, I am checking what R can actually see from that location. If the file I need is not listed, my next move is to check whether the file is in the right folder and whether the file name matches exactly. 

## Step 2

**Create a Notebook** and check the working directory when you Knit.

🎯 Create an R Notebook and include the same code in a code chunk then test **Knit**.
...
✅ Verify: In the knitted notebook output, look directly under the getwd() and list.files() chunk. Confirm that the folder path matches where you saved chapter7_practice.Rmd. Then check whether the file list includes the notebook file itself.
...
Error in setwd("C:/Users/Eva Lorraine Gaudio/OneDrive/Documents/BSU/Teaching/Data-R155/Data-R155/Module_4") : 
...
🎯  Download the data files for this chapter and place them in the same folder as your R Notebook. Then use `list.files()` to check that R can see the files. *This is Task 2 of the Practice Space at the end of this chapter.*
...
🎯 Check that R can see the files you downloaded by running `list.files()` in your R Notebook.
